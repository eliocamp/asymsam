...
indices_2d_yearly %>% 
  ...
  .[, term := factor(term, levels = c("full", "asym", "sym"), ordered = TRUE)] %>% 
  ggplot(aes(lon, lat)) +
...
table <- enso_cors %>% 
  ...
  .[, cor_text := paste0(round(cor, 2), "\n(", p.value, ")")] %>% 
  .[, term := lab_sam[term]] %>% 
  dcast(season ~ term, value.var = "cor_text") %>% 
...
#   # .[, p.value := scales::pvalue(p.value)] %>% 
#   # .[, cor_text := paste0(round(cor, 2), "\n(", p.value, ")")] %>% 
#   dcast(type + term ~ season, value.var = "p.value") %>% 
#   .[, term := lab_sam[term]] 
table2 <- enso_cors %>% 
  .[index == "oni"] %>% 
  .[, p.value := p.adjust(p.value, "fdr")] %>% 
...
table %>% 
  kbl(escape = FALSE,
      ...
      col.names = c("", lab_sam)) %>% 
  column_spec(2, bold = table2[, 2] < 0.05) %>%
  column_spec(3, bold = table2[, 3] < 0.05) %>%
  column_spec(4, bold = table2[, 4] < 0.05) %>%
  kable_paper(full_width = F) %>% 
  add_header_above(c(" " = 1, "Correlation" = 1 , "Partial correlation" = 2)) 
```
