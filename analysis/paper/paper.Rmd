...
indexes %>% 
  ...
  facet_wrap(lev~., labeller = labeller(lev = lev.lab), ncol = 1)
```
...
```{r}
air_regr <- GISTEMP() %>% 
  ReadNetCDF(subset = list(time = range(indexes$time))) %>% 
  normalise_coords() %>% 
  # .[, air := Anomaly(air), by = .(lon, lat)] %>%
  .[indexes_wide[lev == 700], on = .NATURAL] %>% 
  .[, FitLm(air, asym, sym), by = .(lon, lat)] %>% 
  rm_intercept()
```
...
air_regr %>% 
  ...
  scale_x_longitude() +
  scale_y_latitude() +
  scale_fill_divergent("Surface temperature", limits = c(-0.5, 0.5), 
                       oob = scales::squish,
