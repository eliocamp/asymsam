...
```{r vertical-regression, fig.cap = "Asymmetric coefficient of the multiple regression of mean monthly geopotential height anomalies between 65 and 40 South. Values are standarised by the standard deviation at each level. (this caption needs some love)", fig.width = 4, fig.height=3}
ReadNetCDF(ERA5(), 
           subset = list(time = c("1979-01-01", "2018-12-31"),
                         latitude = c(-65, -40)),
           vars = c(hgt = "z")) %>% 
  normalise_coords() %>% 
  .[, .(hgt = mean(hgt)), by = .(lon, lev, time)] %>% 
  .[, hgt := hgt/9.8] %>% 
...
vertical_regr %>% 
  ...
  ggplot(aes(lon, lev)) +
  geom_contour_fill(aes(z = estimate), breaks = AnchorBreaks(0, 0.5, 0)) +
  scale_fill_divergent(NULL, guide = guide_colorstrip_bottom(15), 
                       oob = scales::squish,
                       breaks = MakeBreaks(binwidth = 0.5, exclude = 0)) +
  scale_y_level() +
  scale_x_longitude() 
```
