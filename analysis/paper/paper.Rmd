...
indexes %>% 
  .[lev %in% c(30, 700)] %>% 
  # .[, estimate_norm := estimate_norm/sd(estimate_norm), by = .(lev, term)] %>% 
  .[, fftspectrum(estimate_norm, 5, B = 5000), by = .(term, lev)] -> spec
```

```{r A2, fig.cap = "Fourier spectrum of each timeseries. The shading indicates de 95\\% qunatile derived fitting an AR process and bootstrapping 5000 simulated samples.", appendix = TRUE}
spec %>%
  ggplot(aes(1/freq/12, spec)) +
  geom_area(aes(fill = term, y = ar_spectrum_upp), position = "dodge", alpha = 0.2) +
  # geom_line(aes(color = term, y = ar_spectrum), alpha = 0.5) +
  geom_line(aes(color = term)) +
