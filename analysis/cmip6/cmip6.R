...
if (file.exists(sam_rds)) {
  ...
  compute_asymsam <- function(file, member = 1) {
    ...
    sam <- lapply(plevs, function(plev) {
      ...
      w <- setNames(sqrt(cos(dims$lat*pi/180)), dims$lat)
      data_norm <- apply(data, c("time", "ensemble", "lon", "plev"), function(x) {
        x*w
      })

      data_norm <- aperm(data_norm, c("time", "ensemble", "lon", "lat", "plev"))


      dim(data_norm) <- list(prod(lengths(dimnames(data_norm))[1:2]),
                             prod(lengths(dimnames(data_norm))[3:5]))


      svd <- irlba::irlba(data_norm, 1, 1, rng = runif)
      sam <- svd$v[, 1]
      remove(data_norm)
